...
// exports.l_pos = require('./types_area');

// make l_pos inherent all properties of point2d
l_pos.prototype = new point2d();

var l_area = exports.area = function (center, radius, points) {
    if (points === undefined) { // it's a circle
        ...
        // Convert points to l_pos instances
        this.points = points.map(p => {
            var pos = new l_pos(0, 0);
            pos.parse(p);
            return pos;
        });

        // Calculate the centroid of the points
        var avgX = this.points.reduce((sum, p) => sum + p.x, 0) / this.points.length;
        var avgY = this.points.reduce((sum, p) => sum + p.y, 0) / this.points.length;

        this.center = new l_pos();
        this.center.parse(avgX, avgY);
        // Sort the points according to their angles from the centroid
    ...
    this.coversPoint = function (pos) {
        if (this.isPolygon == true) { // polygon
            ...
            // current check allows nodes even at AOI boundary to be considered as covered
            return (this.center.distance(pos) <= this.radius);
        }
    }

    this.intersectsArea = function (area){
        if (this.isPolygon == true && area.isPolygon == true) { // both polygons
            // separating axis theorem
            // this is a simplification and may not cover all cases
            var polygons = [this.points, area.points];
            ...
            for (var i = 0; i < polygons.length; i++) {
                ...
                for (var i1 = 0; i1 < polygon.length; i1++) {
                    ...
                    for (j = 0; j < polygon.length; j++) {
                        projected = normal.x * polygon[j].x + normal.y * polygon[j].y;
                    ...
                    var polygonB = polygons[(i + 1) % polygons.length];
                    minB = maxB = undefined;
            ...
            return true;
        } else if (this.isPolygon == false && area.isPolygon == true) { // circle vs. polygon
            return circleIntersectsPolygon(this, area);
        } else if (this.isPolygon == true && area.isPolygon == false) { // polygon vs. circle
            return circleIntersectsPolygon(area, this);
        } else { // both circles
        ...
        function circleIntersectsPolygon(circle, polygon) {
            ...
            for (let i = 0; i < polygon.points.length; i++) {
                if (circle.coversPoint(polygon.points[i])) {
                    // console.log("True on check 1");
                    return true;
                } else {
                    // console.log("False on check 1");
                }
            }
            ...
            for (let i = 0, j = polygon.points.length - 1; i < polygon.points.length; j = i++) {
                ...
                if (lineIntersectsCircle(xi, yi, xj, yj, circle.center.x, circle.center.y, circle.radius)) {
                    // console.log("True on check 2");
                    return true;
                } else {
                    // console.log("False on check 2");
                }
            }
        
            // Check if the circle is entirely inside the polygon
            if (polygon.coversPoint(circle.center)) {
                // console.log("True on check 3");
                return true;
            } else {
                // console.log("False on check 3");
            }
        ...
        function lineIntersectsCircle(x1, y1, x2, y2, cx, cy, r) {
            ...
            // Check if projection falls outside of line segment
            if (dot < 0) {
                closestX = x1;
                closestY = y1;
            } else if (dot > 1) {
    ...
    this.equals = function (other_area) {
        if (this.points !== undefined && other_area.points !== undefined) { // both polygons
            if (this.points.length !== other_area.points.length) return false;
            for (var i = 0; i < this.points.length; i++) {
                if (!this.points[i].equals(other_area.points[i])) return false;
            }
            return true;
        } else { // either or both circles
            return (this.center.equals(other_area.center) && this.radius === other_area.radius);
        }
    }
     
    this.toString = function () {
        if (this.points !== undefined) { // polygon
            return 'area: ' + this.points.map(p => p.toString()).join(', ');
        } else { // circle
    ...
    // update info from existing object
    this.update = function(new_info) {
        if (this.points !== undefined && new_info.points !== undefined) { // both polygons
            this.points = new_info.points.map(p => {
                var pos = new l_pos(0, 0);
                pos.update(p);
                return pos;
            });
        } else if (new_info.hasOwnProperty('center') && new_info.hasOwnProperty('radius')) { // circle
            this.center.update(new_info.center);
        
            // only update if radius is valid
            if (new_info.radius >= 0)
    ...
    this.parse = function (js_obj) {
        try {
            if (js_obj.isPolygon) { // polygon
                console.log('Is poly jip')
                this.points = js_obj.points.map(p => {
                    var pos = new l_pos();
                    pos.parse(p);
                    return pos;
                });
                this.isPolygon = js_obj.isPolygon;
                this.center = new l_pos();
                this.center.parse(js_obj.center);
                
                this.radius = js_obj.radius;
            } else if (!js_obj.isPolygon) { // circle
                new_pos = {x: js_obj.center.x, y: js_obj.center.y};
                this.center = new l_pos(0,0);
                this.center.parse(new_pos);
                this.isPolygon = false;
                this.points = undefined;
...
// const l_area = require('./types_area');
// exports.area = require('./types_area');
...
var l_pub = exports.pub = function(matcherID, clientID, aoi, payload, channel){
    ...
    this.parse = function(js_obj){
        ...
        this.unique_id = js_obj.unique_id;
    }
}
...
var l_sub = exports.sub = function (hostID, hostPos, clientID, subID, channel, aoi) {
    ...
    this.parse = function (js_obj) {
        try {
            ...
            this.followClient = js_obj.followClient;
        }
        catch (e) {
