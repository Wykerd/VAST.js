...
function matcher(x, y, radius, opts, onJoin) {
		...
		switch (pack.type) {
            ...
            case Matcher_Message.MOVE_CLIENT : {
                ...
                if (pack.sender !== _id){
                    ...
                    // TODO: clientAccept can be used to balance load/indicate overload
                    ...
                    let new_pack = new VAST.pointPacket(Matcher_Message.MOVE_CLIENT_R, new_msg, _id, _pos, pack.sourcePos)
            ...
            // A client is moving out of my region, i must migrate them to a new matcher
            case Matcher_Message.MOVE_CLIENT_R :{
                ...
                if (_clientList[msg.clientID] !== undefined && _clientID2socket[msg.clientID] !== undefined) {
                    ...
                    // I am receiving the details of the matcher to which this client must transfer.
                    // send the client's subs to the new matcher to prepare

                    if (msg.clientAccept == true){ 
                        ...
                        let new_pack = new VAST.pointPacket(Matcher_Message.MOVE_SUBS, new_msg, _id, _pos, pack.sourcePos);
                        _vonPeer.pointMessage(new_pack);
                    }
                    // TODO: Client not accepted / overload procedure
                    //else 
		...
		_io.on('connection', function(socket){
			...
			socket.on('subscribe', function(msg) {
                ...
                if (msg.hasOwnProperty('points')) { // polygon
                    _subscribe(clientID, msg.points, msg.channel, msg.followClient);
                } else { // circle
                    _subscribe(clientID, {x: msg.x, y: msg.y, radius: msg.radius}, msg.channel, msg.followClient);
                }
            });
    ...
    var _moveClient = function(clientID, x, y){
        ...
        for (var subID in subs){
            ...
            if (sub.followClient === true){
                console.log('I am following the client')
                // Now we must move the sub to the new position
                // if (sub.aoi.isPolygon === false) {
                    _moveSubscription(sub, new VAST.pos(delta_x,delta_y));
                // };
					...
					if(sub.hostID === _id){
						...
						try {
							...
							if (_useMQTT == true) {
                                ...
                                tempcounter += 1;
                                // console.log('I am sending this amount of publications to clients: ' + tempcounter)
    ...
    var _subscribe = this.subscribe = function(clientID, area, channel, followClient) {
        ...
        if (_useMQTT == false) {
            try {
                ...
                log.error("Matcher["+_id+"]: no socket for Client["+clientID+"]");
            }
        }
    }
	...
	var _addSubscription = function (sub, forward) {
