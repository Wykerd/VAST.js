...
const { decodeBinary: decodeClientProto } = require('./proto/generated/messages/VASTClientMessage.js');
const { encodeBinary: encodeServerProto } = require('./proto/generated/messages/VASTServerMessage.js');
...
function matcher(x, y, radius, opts, onJoin) {
	...
	const _http = createServer();
	const _io = createSocketIO(_http, {
		cors: {
			origin: "*"
		}
	});
     ...
     * @returns {boolean}
     */
    function canConvertToUint8Array(arr) {
    ...
    function convertToBuffer(data) {
        const canBeConverted = canConvertToUint8Array(data);
        if (canBeConverted) return new Uint8Array(data);
     ...
     * @param {WebSocket} ws 
     */
    function createEmitWrapper(ws, excludePublicationInfo = false, excludeChannel = false) {
        
        /**
         * @param {'confirm_matcher' | 'assign_matcher' | 'publication' | 'unsubscribe_r' | 'subscribe_r'} event 
         * @param {*} data 
         */
        function emit(event, data) {
            switch (event) {
                case 'confirm_matcher':
                    ws.send(encodeServerProto({
                        message: {
                            field: 'confirmMatcher',
                            value: {
                                clientId: data.clientID,
                                matcherId: data.matcherID.toString(),
                                matcherAddr: {
                                    host: data.matcherAddr.host,
                                    port: data.matcherAddr.port,
                                },
                                matcherPosition: {
                                    x: data.matcherPos.x,
                                    y: data.matcherPos.y,
                ...
                case 'publication':
                    {
                        const payload = convertToBuffer(data.payload);
                        ...
                        ws.send(encodeServerProto({
                            message: {
                                ...
                                value: {
                                    payload,
                                    aoi: excludePublicationInfo ? undefined : data.aoi.isPolygon ? {
                                        field: 'polygon',
                                        value: {
                                            points: data.aoi.points.map(point => ({ x: point.x, y: point.y }))
                                        }
                                    } : {
                                        field: 'circular',
                                        value: {
                                            ...
                                            radius: data.aoi.radius
                                        }
                                    },
                                    channel: excludeChannel ? undefined : data.channel,
                                }
                            }
                        }));
                    }
    ...
    server.on('upgrade', (request, socket, head) => {
        const { searchParams } = new URL(request.url ?? '/', 'wss://base.url');

        const clientID = searchParams.get('id');
        if (!clientID) 
            return socket.destroy();
        const options = new Set(searchParams.getAll('options').join(',').toLowerCase().split(','));
        const position = searchParams.get('pos').split(',').map(Number);
        if (position.length !== 2 || position.some(Number.isNaN)) 
            return socket.destroy();

        console.log(`Matcher[${_id}]: New client connection with id=${clientID}, position=(${position.join(',')}), options=${[...options].join(', ')}`);
        ...
        wss.handleUpgrade(request, socket, head, ws => {
            ...
            if (_id  == VAST.ID_GATEWAY){
                ...
                // Add to client list
                var client = _addClient(clientID, {
                    x: position[0],
                    y: position[1]
                })
                ...
                _clientID2socket[clientID] = {
                    id: Symbol('ws-client'),
                    emit: createEmitWrapper(ws, options.has('nopubinfo'), options.has('nochannel')),
                };
                _clientID2socket[clientID].emit('confirm_matcher', client_pack);

                // record event
                _recordEvent(Matcher_Event.CLIENT_JOIN, client);
                log.debug('Matcher['+_id+']: a new client has joined GW. Send MOVE request');
                _moveClient(clientID, position[0], position[1]);
            }
            ...
            ws.on('message', (message) => {
                const arrayBuffer = convertToArrayBuffer(message);

                const payload = decodeClientProto(new Uint8Array(arrayBuffer));

                if (!payload.message) return;

                switch (payload.message.field) {
                    case 'publish':
                        {
                            if (payload.message?.value.aoi?.field !== 'circular') return;
                            const { aoi, channel, payload: bufLoad } = payload.message.value;
                            if (!aoi.value.center || !channel) return;
                            _publish(clientID, aoi.value.center.x, aoi.value.center.y, aoi.value.radius, Array.from(bufLoad), channel, crypto.randomUUID());
                        }
                        break;

                    case 'subscribe':
                        {
                            const { id: subID, channel, follow, aoi } = payload.message.value;
                            if (!aoi) return;
                            log.debug('Matcher['+_id+']: Received subscribe message from client['+clientID+'] with subscription ID ['+subID+']');
                            if (aoi.field === 'circular')
                                _subscribe(clientID, {
                                    x: aoi.value.center.x,
                                    y: aoi.value.center.y,
                                    radius: aoi.value.radius
                                }, channel, subID, follow);
                            else 
                                _subscribe(clientID, aoi.value.points, channel, subID, follow);
                        }
                        break;
                    ...
                    case 'unsubscribe':
                        {
                            ...
                            _unsubscribe(clientID, payload.message.value);
                        }
                        break;

                    case 'move':
                        {
                            _moveClient(clientID, payload.message.value.x, payload.message.value.y);
                        }
                        break;
    ...
    server.listen(_opts.client_port_ws);
    ...
    this.getInfo = () => {
        return {
            ...
            aoi: _aoi,
            pos: _pos,
            subscriptions: Object.values(_subscriptions).flatMap(subs => Object.values(subs)),
            clientList: Object.values(_clientList),
        };
    }
    ...
    this.handlePacket = function(pack){
        ...
        switch (pack.type) {
            ...
            case Matcher_Message.PUB_MATCHED: {
                ...
                log.debug('Matcher['+_id+']: Received Matched Publication <'  + msg.pubID + '> from Matcher['+pack.sender+']' + 'from Client['+pack.msg.clientID+']');
    
                var pub = msg;

                // Only for logging purposes - add pack.chain (the hops from pub host to me) to the pub.chain (the hops from pub origin to pub host):
                // First pop the last element of the chain (which is the turn around point, i.e. it will be the first element of the new chain)
                // pub.chain.pop(); // We don't pop so that we can see the full chain - also the turn around point
                pub.chain.push(...pack.chain); 
                // pub.recipients = pack.recipients;
            ...
            case Matcher_Message.SUB_NEW: {
                ...
                // If the sub received is for an clientID which i host, i.e. in _clientList, ignore. This packet must of originated from me or migrated from me
                // if (_clientList[msg.clientID] !== undefined) {
                //     log.debug('Matcher['+_id+']: Ignored new sub from Matcher['+pack.sender+'] for client['+msg.clientID+']');
                //     break;
                // }

                // if subID in my subslist, just update sub

                log.debug('Matcher[' + _id + ']: New Sub from Matcher[' + pack.sender + '] with subID: ' + msg.subID, msg);
                var sub = msg;

                if (_subscriptions.hasOwnProperty(sub.clientID) && _subscriptions[sub.clientID].hasOwnProperty(sub.subID)){ // if the subID already exists, update the subscription rather than adding a new one
            ...
            case Matcher_Message.MOVE_CLIENT: {
                ...
                // Respond with a MOVE_SUBS_R message
                if (pack.sender !== _id && _pendingClients[msg.clientID] === undefined) {
                    ...
                    delete _leavingClientDeleteFunction[msg.clientID];
                    ...
                    for (var subID in msg.clientSubs) {
                        ...
                        msg.clientSubs[subID].hostPos = _pos;
            ...
            case Matcher_Message.MOVE_SUBS_R: {
                log.debug('Matcher[' + _id + ']: Received MOVE_SUBS_R message');
                ...
                // }
                try {
                    delete _subscriptions[msg.clientID];
                } catch (error) {
                    log.debug('Matcher[' + _id + ']: Error deleting subscriptions for client[' + msg.clientID + ']');
                }
                ...
                if (_useMQTT == true) {
                    ...
                    if (_clientList[msg.clientID] !== undefined && _clientID2socket[msg.clientID] !== undefined) {
                        ...
                        log.debug('Matcher[' + _id + ']: client[' + msg.clientID + '] has already left');
                    }
                }
                break;
            }
        ...
        _io.on('connection', function(socket) {
            ...
            socket.on('ping', function(data) {
                ...
                _publish(clientID, data.x, data.y, data.radius, 
                    data.payload, data.channel, data.pubID);
                ...
                pub = new VAST.pub(_id, clientID, data.pubID ,aoi, pongData.payload, pongData.channel);

                // console.log('I am sending a pong to the client with data: ' + data.pubID)
    ...
    const emitter = this.events = new EventEmitter();
    ...
    var _recordEvent = function(event, msg){
        ...
        switch (event){
            ...
            case Matcher_Event.MATCHER_JOIN :{
                ...
                emitter.emit('matcher-event', data);
            }
            break;
            ...
            */
            ...
            case Matcher_Event.DISSEMINATE :{
                var data = {
                    time : UTIL.getTimestamp(),
                    event : Matcher_Event.DISSEMINATE,
                    id : _id,
                    alias : _alias,
                    sub : msg.sub,
                    pub : msg.pub
                }
                // events.printObject(data); XXX: not handled yet
  ...
  var _addMQTTClient = function(mqttID, clientPos) {
	...
	var _publish = this.publish = function(clientID, x, y, radius, payload, channel, pubID, done) {
		...
		//inform GS before sending the publication (for event ordering)
  ...
  var _sendPublication = this.sendPublication = function(publication, allowForwarding) {
      ...
      //console.log(clientSubs);
			...
			*/
            // Do not send to self if self is augeo client
            if ((clientID == pub.clientID) && pub.clientID.includes('AugeoClient')) {
                continue;
            }
				...
				if(sub.aoi.intersectsArea(pub.aoi)){
					...
					if(sub.hostID === _id){
						...
						try {
                            ...
                            if (pub.payload.username?.split("&")[1]) {
                                ...
                                // try{
                                //     log.debug("Sending packet <" + pub.pubID + "> to client <" + sub.clientID + "> after " + pub.chain.length + " hops; hops: " + pub.chain);
                                // } catch {
                                //     console.log("Error in logging hops")
                                // }
                            }

                            _recordEvent(Matcher_Event.DISSEMINATE, {
                                sub, pub
                            });
					...
					// the nearest recipient to the host
                        ...
                        if (allowForwarding === true && pub.recipients.includes(sub.hostID) === false
                            ...
                            // pub.hops_to_host = pub.chain.length// TODO: Finish

                            pointPacket = new VAST.pointPacket(Matcher_Message.PUB_MATCHED, pub, _id, _pos, sub.hostPos);

                            log.debug('Matcher['+_id+']: Sending Matched Publication <' + pub.pubID + '> to hostPOS: ' + sub.hostPos.x + ', ' + sub.hostPos.y);
    ...
    var _handlePublication = this.handlePublication = function(publication){
		...
		for (var clientID in _subscriptions) {
			...
			*/
            // Do not send to self if I'm a Augeo client
            if((clientID == pub.clientID) && pub.clientID.includes("AugeoClient")) {
                continue;
            }
