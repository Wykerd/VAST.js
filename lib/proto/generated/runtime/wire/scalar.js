...
exports.packFns = exports.unpackFns = exports.tsValueToWireValueFns = exports.wireValueToTsValueFns = void 0;
const Long_js_1 = __importDefault(require("../Long.js"));
const varint_js_1 = require("./varint.js");
const zigzag_js_1 = require("./zigzag.js");
...
const decodeVarintFns = {
    ...
    sint32: (long) => (0, zigzag_js_1.decode)(long[0]),
    sint64: (long) => (0, zigzag_js_1.decode)(long).toString(true),
    bool: (long) => long[0] !== 0,
};
const encodeVarintFns = {
    int32: (tsValue) => new Long_js_1.default(tsValue),
    int64: (tsValue) => Long_js_1.default.parse(tsValue),
    uint32: (tsValue) => new Long_js_1.default(tsValue),
    uint64: (tsValue) => Long_js_1.default.parse(tsValue),
    sint32: (tsValue) => (0, zigzag_js_1.encode)(new Long_js_1.default(tsValue)),
    sint64: (tsValue) => (0, zigzag_js_1.encode)(Long_js_1.default.parse(tsValue)),
    bool: (tsValue) => new Long_js_1.default(+tsValue),
};
const varintFieldToTsValueFns = Object.fromEntries(Object.entries(decodeVarintFns).map(([type, fn]) => [
    type,
    (wireValue) => {
        if (wireValue.type !== index_js_1.WireType.Varint)
            return;
...
const tsValueToVarintFieldFns = Object.fromEntries(Object.entries(encodeVarintFns).map(([type, fn]) => ([
    type,
    (tsValue) => ({
        type: index_js_1.WireType.Varint,
        value: fn(tsValue),
    }),
])));
exports.wireValueToTsValueFns = {
    ...varintFieldToTsValueFns,
    double: (wireValue) => {
        if (wireValue.type !== index_js_1.WireType.Fixed64)
            return;
        const dataview = new DataView(wireValue.value.buffer);
        return dataview.getFloat64(0, true);
    },
    float: (wireValue) => {
        if (wireValue.type !== index_js_1.WireType.Fixed32)
            return;
    ...
    string: (wireValue) => {
        if (wireValue.type !== index_js_1.WireType.LengthDelimited)
            return;
...
exports.tsValueToWireValueFns = {
    ...tsValueToVarintFieldFns,
    double: (tsValue) => {
        const long = new Long_js_1.default();
        const dataview = new DataView(long.buffer);
        dataview.setFloat64(0, tsValue, true);
        return { type: index_js_1.WireType.Fixed64, value: long };
    },
    float: (tsValue) => {
        const u32 = new Uint32Array(1);
        const dataview = new DataView(u32.buffer);
        dataview.setFloat32(0, tsValue, true);
        return { type: index_js_1.WireType.Fixed32, value: dataview.getUint32(0, true) };
    },
    fixed32: (tsValue) => ({ type: index_js_1.WireType.Fixed32, value: tsValue >>> 0 }),
    fixed64: (tsValue) => ({
        type: index_js_1.WireType.Fixed64,
        value: Long_js_1.default.parse(tsValue),
    }),
    sfixed32: (tsValue) => ({ type: index_js_1.WireType.Fixed32, value: tsValue | 0 }),
    sfixed64: (tsValue) => ({
        type: index_js_1.WireType.Fixed64,
        value: Long_js_1.default.parse(tsValue),
    }),
    string: (tsValue) => {
        ...
        return {
            type: index_js_1.WireType.LengthDelimited,
            value: textEncoder.encode(tsValue),
        };
    },
    bytes: (tsValue) => ({ type: index_js_1.WireType.LengthDelimited, value: tsValue }),
};
const unpackVarintFns = Object.fromEntries(Object.keys(decodeVarintFns).map((type) => [
    type,
    function* (wireValues) {
        for (const wireValue of wireValues) {
            const value = exports.wireValueToTsValueFns[type](wireValue);
            if (value != null)
                yield value;
            else {
...
exports.unpackFns = {
    ...
    *double(wireValues) {
        for (const wireValue of wireValues) {
            const value = exports.wireValueToTsValueFns.double(wireValue);
            if (value != null)
                yield value;
            else
    ...
    *float(wireValues) {
        for (const wireValue of wireValues) {
            const value = exports.wireValueToTsValueFns.float(wireValue);
            if (value != null)
                yield value;
            else
    ...
    *fixed32(wireValues) {
        for (const wireValue of wireValues) {
            const value = exports.wireValueToTsValueFns.fixed32(wireValue);
            if (value != null)
                yield value;
            else
    ...
    *fixed64(wireValues) {
        for (const wireValue of wireValues) {
            const value = exports.wireValueToTsValueFns.fixed64(wireValue);
            if (value != null)
                yield value;
            else {
    ...
    *sfixed32(wireValues) {
        for (const wireValue of wireValues) {
            const value = exports.wireValueToTsValueFns.sfixed32(wireValue);
            if (value != null)
                yield value;
            else
                for (const value of unpackFixed32(wireValue))
    ...
    *sfixed64(wireValues) {
        for (const wireValue of wireValues) {
            const value = exports.wireValueToTsValueFns.sfixed64(wireValue);
            if (value != null)
                yield value;
            else {
                for (const value of unpackFixed64(wireValue)) {
...
const packVarintFns = Object.fromEntries(Object.keys(encodeVarintFns).map((type) => [
    ...
    function (tsValues) {
        return {
            type: index_js_1.WireType.LengthDelimited,
            value: (0, serialize_js_1.concat)(tsValues.map((tsValue) => {
                const value = encodeVarintFns[type](tsValue);
                return (0, varint_js_1.encode)(value);
            })),
        };
    },
]));
function getFixedPackFn(size, setFn) {
    return function pack(values) {
        ...
        return { type: index_js_1.WireType.LengthDelimited, value };
    };
}
exports.packFns = {
    ...packVarintFns,
    ...
    fixed64: getFixedPackFn(8, (dataView, byteOffset, value) => {
        const long = Long_js_1.default.parse(value);
...
function* unpackVarint(wireValue) {
    ...
    while (idx < value.length) {
        const decodeResult = (0, varint_js_1.decode)(new DataView(value.buffer, offset + idx));
        idx += decodeResult[0];
...
function* unpackFixed64(wireValue) {
    ...
    while (idx < value.length) {
        ...
        idx += 4;
        yield new Long_js_1.default(lo, hi);
    }
}
